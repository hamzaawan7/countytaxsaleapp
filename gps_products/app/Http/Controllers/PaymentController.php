...
$end_date=$subscription->current_period_end;
  ...
  $card_type=$customer->sources->data[$i]->brand; 
  ...
  if(count($c)==0){
    ...
    $body=$e->getMessage();
        ...
        return redirect()->route('dashboard')->with('success', $body);
...
DB::table('debug')->insert([
   ...
   $event_json = json_decode($input);
    ...
    // $defbug = $users = DB::table('debug')->where('debug_id',12)->first();
    // $event_json = json_decode($defbug->json_content);
...
$plan_name=$event_json->data->object->lines->data[0]->plan->name;
...
$pr = Premium_user::where('user_id',$user_id)->first();
   if($pr){
$status=$pr->status;

 $t = $users = DB::table('premium_users')->where('user_id', $user_id)->orderBy('id','DESC')->first();
      $trial_days=0;
       if($t){
         $current_date_time=date('Y-m-d H:i:s');  
       //$current_date=date('Y-m-d');
     ...
     if($status=="trial" && $trial_days==0){

$sub = \Stripe\Subscription::retrieve($subscription_id);
$sub->cancel();

$plan_id="";
$amount_payment=$dynemic_charge;
       try {
...
if($plan_name=="premiumall" && $amount!=0){
