@extends('layouts.theme_main')
@section('title', 'County Tax Sale App (CTSA) | Forgot')
@section('content')


    <div class="col-md-4">
    </div>
    <div class="mr-front-content col-xs-12 col-sm-12 col-md-4 col-lg-4">
        <div class="locksreen-col text-center  wow fadeInUp">
            {{ csrf_field() }}
            <a href="{{ route('forgot-username') }}" class="btn btn-blue btn-lg btn-block ">Forgot Username</a>
            <br/>
            <a href="{{ route('password.request') }}" class="btn btn-blue btn-outline btn-lg btn-block ">Forgot
                Password</a>
        </div>
    </div>
    <div class="col-md-4">
    </div>

@stop