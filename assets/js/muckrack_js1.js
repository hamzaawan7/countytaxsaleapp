...
;$(function () {
    $('.js-mr-searchform-navbar').submit(function (e) {
        var newQstring = '?', q = encodeURIComponent($(this).find('input[name="q"]').val());
        params = qmark();
        if (q == '') {
            e.preventDefault();
            return;
        ...
        params['q'] = q;
        var qStringPairs = _.pairs(params);
        for (var i = 0; i < qStringPairs.length; i++) {
            var item = '';
            if (i > 0) {
                item = '&';
            }
            item = item + qStringPairs[i][0] + '=' + qStringPairs[i][1];
            newQstring = newQstring + item;
        }
        window.location = '/search/results' + newQstring;
        e.preventDefault();
    });
});
;$(function () {
    var searchformInput = $('.js-mr-searchform-input');
    var searchformInputWrapperClass = '.js-mr-searchform-input-wrapper';
    searchformInput.on('input', function () {
        var $this = $(this);
        $this.closest(searchformInputWrapperClass).removeClass('open');
        $this.attr('aria-expanded', 'false');
        if ($this.val() === '') {
            $this.closest(searchformInputWrapperClass).addClass('open');
            $this.attr('aria-expanded', 'true');
        }
    });
    var $appear_in_checkboxes = $('#div_id_search_type').find('[value=names], [value=bios]').parent();
    if ($('#id_person').val()) {
        $appear_in_checkboxes.addClass('hidden');
    }
    $('.js-person-clear').on('click', function (event) {
        event.preventDefault();
        $('.js-mr-person').addClass('hidden');
        $('.js-mr-person-type').removeClass('hidden');
        $appear_in_checkboxes.removeClass('hidden');
        $('#id_person').val('');
    });
});